...
class ModelData(object):
    ...
    def load_net_model_path(self, path):
        ...
        # If the state dict keys start with "module.", we remove that prefix
        ...
        # Now load the (potentially modified) state dict
        net.load_state_dict(new_state_dict)

        # Right before `net.load_state_dict(new_state_dict)`
        net.eval()
        
        print("Model loaded in {:.4f} seconds.".format(time.time() - model_loading_start_time))
        return net
